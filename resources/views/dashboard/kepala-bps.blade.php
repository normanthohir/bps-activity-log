        ...
        <div
            ...
            <div class="relative flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p class="text-white/70 text-sm mt-1">Akses seluruh bagian &middot; Rekap dan persetujuan lintas
                        seksi</p>
                </div>
                <x-action-button href="{{ route('kepala-bps.tugas.create') }}" variant="light">
                    Beri Tugas
                </x-action-button>
                ...
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-lg bg-indigo-50 flex items-center justify-center shrink-0">
                        <svg class="w-5 h-5 text-indigo-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                            stroke-width="1.5">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                d="M2.25 21h19.5m-18-18v18m2.25-18v18M13.5 3v18m3.75-18v18m2.25-18v18M6.75 6.75h.75m-.75 3h.75m-.75 3h.75m3-6h.75m-.75 3h.75m-.75 3h.75M6.75 21v-3.375c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21" />
                        </svg>
                    </div>
                    <div>
                    ...
                    @if ($totalDilihat > 0)
                        ...
                        <div class="flex flex-wrap gap-x-5 gap-y-1.5 mt-3 text-xs text-gray-600">
                            <span class="flex items-center gap-1.5"><span
                                    class="w-2 h-2 rounded-full bg-green-500"></span> Disetujui
                                ({{ $disetujuiBulanIni }})</span>
                            <span class="flex items-center gap-1.5"><span
                                    class="w-2 h-2 rounded-full bg-amber-400"></span> Menunggu
                                ({{ $sisaMenunggu }})</span>
                            <span class="flex items-center gap-1.5"><span
                                    class="w-2 h-2 rounded-full bg-red-400"></span> Dikembalikan
                                ({{ $dikembalikanBulanIni }})</span>
                        </div>
                    @else
                ...
                <div class="animate-in delay-3 bg-white rounded-xl border border-gray-200 overflow-hidden">
                    <div
                        ...
                        <a href="{{ route('kepala-bps.rekap') }}"
                            class="text-sm text-[#1F3864] hover:underline font-medium">Lihat Detail</a>
                    </div>
                            ...
                            <tbody class="divide-y divide-gray-100">
                                @foreach ($rekapPerBagian as $i => $bagian)
                                    <tr class="animate-row hover:bg-gray-50/70 transition-colors"
                                        ...
                                        <td class="py-3 px-5 font-medium text-gray-900">{{ $bagian->nama_bagian }}
                                        </td>
                                        <td class="py-3 px-5 text-right text-gray-500">{{ $bagian->pegawai_count }}
                                        </td>
                                        <td class="py-3 px-5 text-right text-gray-500">
                                            {{ $bagian->total_laporan_bulan_ini }}</td>
                                        <td class="py-3 px-5 text-right">
                                            @if ($bagian->menunggu_approval > 0)
                                                <span
                                                    class="inline-flex items-center gap-1 bg-amber-50 text-amber-700 text-xs font-medium px-2 py-0.5 rounded-full ring-1 ring-inset ring-amber-600/20">
                        ...
                        <div class="divide-y divide-gray-100">
                            @foreach ($laporanMenunggu as $laporan)
                                <a href="{{ route('kepala-bps.approval.show', $laporan) }}"
                                    ...
                                    <span
                                        class="w-7 h-7 rounded-full bg-[#DCE6F1] text-[#1F3864] text-xs font-semibold flex items-center justify-center shrink-0">
                                    ...
                                    <div class="min-w-0">
                                        ...
                                        <p class="text-xs text-gray-500 truncate">
                                            {{ $laporan->user->bagian->nama_bagian ?? '-' }} &middot;
                    ...
                    @else
                        <div class="divide-y divide-gray-100">
                            @foreach ($tugasSayaBerikan as $tugas)
                                ...
                                @endphp
                                <a href="{{ route('kepala-bps.tugas.show', $tugas) }}"
                                    ...
                                    <div class="flex items-center justify-between gap-2">
                                        ...
                                        <span
                                            class="inline-flex items-center gap-1 {{ $badge[0] }} text-[10px] font-medium px-2 py-0.5 rounded-full ring-1 ring-inset shrink-0">
                                            <span class="w-1.5 h-1.5 rounded-full {{ $badge[1] }}"></span>
                                            {{ $badge[2] }}
                                        </span>
                                    </div>
                                    <p class="text-xs text-gray-500 mt-0.5 truncate">
                                        {{ $tugas->penerimaTugas->name }} &middot;
