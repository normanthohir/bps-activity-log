            ...
            <div class="hidden md:flex items-center" x-data="{ open: false }">
                ...
                <div x-show="open" x-cloak x-transition:enter="transition ease-out duration-100"
                    ...
                    <form method="POST" action="{{ route('logout') }}" x-data="{ loading: false }"
                        @submit="loading = true">
                        @csrf
                        <x-loading-overlay message="Mengeluarkan akun..." />
                        <button type="submit" :disabled="loading"
                            class="flex items-center gap-2 w-full px-4 py-2.5 text-sm text-red-600 hover:bg-red-50 disabled:opacity-70 disabled:cursor-not-allowed transition-colors">
                            <x-button-spinner x-show="loading" x-cloak class="text-red-600" />
                            <svg x-show="!loading" class="w-4 h-4" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round"
                                    d="M15.75 9V5.25A2.25 2.25 0 0013.5 3h-6a2.25 2.25 0 00-2.25 2.25v13.5A2.25 2.25 0 007.5 21h6a2.25 2.25 0 002.25-2.25V15m3 0l3-3m0 0l-3-3m3 3H9" />
                            </svg>
                            <span x-text="loading ? 'Keluar...' : 'Keluar'"></span>
                        </button>
                    </form>
                </div>
            </div>
