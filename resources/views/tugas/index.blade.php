...
<x-app-layout>
    ...
    <div class="max-w-5xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        ...
        {{-- Filter card --}}
        <form method="GET" action="{{ route('kabag.tugas.index') }}"
            ...
            @if ($tugas->isEmpty())
                <div class="text-center py-16 px-4">
                    ...
                    <p class="text-sm font-medium text-gray-900 mt-3">
                        {{ request()->hasAny(['cari', 'status']) ? 'Tidak ada tugas yang cocok' : 'Belum ada tugas' }}
                    </p>
                    <p class="text-sm text-gray-500 mt-1">
                        {{ request()->hasAny(['cari', 'status']) ? 'Coba ubah kata kunci atau filter.' : 'Buat tugas baru untuk diberikan ke staf.' }}
                    </p>
                </div>
            @else
                                        ...
                                        <div class="flex items-center justify-end gap-2">
                                            <x-dropdown-action>
                                                ...
                                                @if ($item->status === 'belum_dikerjakan')
                                                    <a href="{{ route('kabag.tugas.edit', $item) }}"
                                                        class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                                                        Ubah Data
                                                    </a>
