...
<nav class="bg-[#1F3864] shadow-lg sticky top-0 z-50">
        ...
        <div class="flex justify-between h-16 items-center">
            ...
            <div class="flex items-center gap-3">
                <a href="{{ route('dashboard') }}" class="flex items-center gap-2.5">
                    ...
                    <span class="text-white font-semibold text-sm tracking-wide hidden sm:block">RAPI - BPS Kota Ambon</span>
                </a>
            </div>
            ...
            <div class="hidden md:flex items-center gap-1">
                ...
                <a href="{{ route('dashboard') }}"
                   class="px-3 py-2 rounded-lg text-sm font-medium transition-colors {{ str_starts_with($currentRoute, 'dashboard') ? 'bg-white/15 text-white' : 'text-white/70 hover:text-white hover:bg-white/10' }}">
                    Dashboard
                </a>
                ...
                @if (auth()->user()->isKepalaBagian())
                    <a href="{{ route('kabag.tugas.index') }}"
                       class="px-3 py-2 rounded-lg text-sm font-medium transition-colors {{ str_starts_with($currentRoute, 'kabag.tugas') ? 'bg-white/15 text-white' : 'text-white/70 hover:text-white hover:bg-white/10' }}">
                        Tugas Tim
                    </a>
            ...
            <div class="hidden md:flex items-center" x-data="{ open: false }">
                <button @click="open = !open" @keydown.escape.window="open = false"
                    ...
                    <span class="w-8 h-8 rounded-full bg-white/15 text-white text-xs font-semibold flex items-center justify-center">
    ...
    <div id="mobile-nav" class="hidden md:hidden border-t border-white/10">
        <div class="px-4 py-3 space-y-1">
            <a href="{{ route('dashboard') }}" class="block px-3 py-2 rounded-lg text-sm font-medium text-white/70 hover:text-white hover:bg-white/10">Dashboard</a>
            @if (!auth()->user()->isKepalaBps())
            <a href="{{ route('laporan.index') }}" class="block px-3 py-2 rounded-lg text-sm font-medium text-white/70 hover:text-white hover:bg-white/10">Laporan Saya</a>
            @endif
            @if (auth()->user()->role === 'staf')
            <a href="{{ route('staf.tugas.index') }}" class="block px-3 py-2 rounded-lg text-sm font-medium text-white/70 hover:text-white hover:bg-white/10">Tugas Saya</a>
            @endif
            @if (auth()->user()->isKepalaBagian())
                <a href="{{ route('kabag.tugas.index') }}" class="block px-3 py-2 rounded-lg text-sm font-medium text-white/70 hover:text-white hover:bg-white/10">Tugas Tim</a>
                <a href="{{ route('kabag.approval.index') }}" class="block px-3 py-2 rounded-lg text-sm font-medium text-white/70 hover:text-white hover:bg-white/10">Persetujuan</a>
            @endif
            @if (auth()->user()->isKepalaBps())
                <a href="{{ route('kepala-bps.rekap') }}" class="block px-3 py-2 rounded-lg text-sm font-medium text-white/70 hover:text-white hover:bg-white/10">Rekap Kantor</a>
                <a href="{{ route('kepala-bps.tugas.index') }}" class="block px-3 py-2 rounded-lg text-sm font-medium text-white/70 hover:text-white hover:bg-white/10">Beri Tugas</a>
                <a href="{{ route('kepala-bps.approval.index') }}" class="block px-3 py-2 rounded-lg text-sm font-medium text-white/70 hover:text-white hover:bg-white/10">Persetujuan</a>
            @endif
            @if (auth()->user()->isAdmin())
                <a href="{{ route('admin.users.index') }}" class="block px-3 py-2 rounded-lg text-sm font-medium text-white/70 hover:text-white hover:bg-white/10">Kelola User</a>
                <a href="{{ route('admin.bagian.index') }}" class="block px-3 py-2 rounded-lg text-sm font-medium text-white/70 hover:text-white hover:bg-white/10">Kelola Bagian</a>
            @endif
            <hr class="border-white/10 my-2">
            <a href="{{ route('profile.edit') }}" class="block px-3 py-2 rounded-lg text-sm font-medium text-white/70 hover:text-white hover:bg-white/10">Profile Saya</a>
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" class="block w-full text-left px-3 py-2 rounded-lg text-sm font-medium text-red-400 hover:text-red-300 hover:bg-white/10">Keluar</button>
            </form>
        </div>
    </div>
</nav>
