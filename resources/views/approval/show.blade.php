{{-- resources/views/approval/show.blade.php --}}
<x-app-layout>
    <style>
    ...
    <div class="max-w-2xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        ...
        <div class="animate-in delay-1 bg-white rounded-xl border border-gray-200 p-6 mb-6 transition-all hover:shadow-sm">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4">
                <div class="flex items-center gap-3">
                    <span class="w-10 h-10 rounded-full bg-[#DCE6F1] text-[#1F3864] text-sm font-semibold flex items-center justify-center shrink-0">
            ...
            @if ($laporan->status === 'menunggu')
                <div class="pt-4 border-t border-gray-100" x-data="{ tolak: {{ $errors->has('catatan') ? 'true' : 'false' }} }">
                    ...
                    <form x-show="tolak" x-transition method="POST"
                        action="{{ route('kabag.approval.proses', $laporan) }}"
            ...
            @forelse ($laporan->logApproval as $log)
                <div class="border-t border-gray-100 first:border-t-0 py-3">
                    <div class="flex justify-between items-center">
                        ...
                        <span class="inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full ring-1 ring-inset
                            {{ $log->aksi === 'disetujui' ? 'bg-green-50 text-green-700 ring-green-600/20' : 'bg-red-50 text-red-700 ring-red-600/20' }}">
                            <span class="w-1.5 h-1.5 rounded-full {{ $log->aksi === 'disetujui' ? 'bg-green-500' : 'bg-red-500' }}"></span>
                            {{ ucfirst($log->aksi) }}
                        </span>
                    </div>
