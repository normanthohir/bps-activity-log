...
<x-app-layout>
    <div class="max-w-2xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-xl border border-gray-200 overflow-hidden mb-6">
            <div class="p-6">
                <div class="grid grid-cols-1 sm:grid-cols-1 gap-4 mb-4 pt-4 border-t border-gray-100">
                    <div>
                    ...
                    @foreach ($tugas->laporanHarian as $laporan)
                        <div class="px-5 py-4">
                            <div class="flex justify-between items-center mb-1">
                                <p class="text-sm font-medium text-gray-900">{{ $laporan->tanggal->format('d M Y') }}
                                </p>
                            ...
                            @if ($laporan->status === 'menunggu')
                                <div class="flex gap-2 mt-2">
                                    <form method="POST" action="{{ route('kabag.approval.proses', $laporan) }}">
                                        ...
                                        <button
                                            class="inline-flex items-center gap-1 text-sm font-medium px-3 py-1.5 rounded-lg bg-green-50 text-green-700 hover:bg-green-100 transition-colors">
                                            Setujui
                                        </button>
                                    </form>
                                    <form method="POST" action="{{ route('kabag.approval.proses', $laporan) }}">
        ...
        <a href="{{ route('kabag.tugas.index') }}"
            class="inline-flex mt-3 items-center gap-1.5 bg-[#1F3864]/10 border border-[#1F3864]/30 text-[#1F3864] hover:bg-[#1F3864]/20 text-sm font-medium px-4 py-2 rounded-lg transition-colors">
